...
class Model:
    ...
    def extract(self, outdir: Path = None):
        ...
        counts = self._build_counts(entities, attrs, rels, idents, extended_texts)
        ea_profile = self._build_extended_attribute_profile(extended_texts)
        entities_list = self._build_entities_list(entities)
        ...
        self._write_output_files(outdir, counts, ea_profile, entities_list)

        return data

    def _build_counts(self, entities, attrs, rels, idents, extended_texts):
        c = {
            "Entity": len(entities),
            ...
            "RelationshipJoin": len(self._defs("Join")) + len(self._defs("ReferenceJoin")),
            "InheritanceLink": len(self._defs("InheritanceLink")),
        }
        ...
        c["inheritances_mutually_exclusive"] = len(self.root.findall(".//o:Inheritance[a:MutuallyExclusive='1']", namespaces=NS)) +                                                len(self.root.findall(".//o:Inheritance[a:MutuallyExclusive='true']", namespaces=NS))
        
        c["entities_with_comment"] = len(self.root.findall(".//o:Entity[a:Comment]", namespaces=NS)) +                                      len(self.root.findall(".//o:Table[a:Comment]", namespaces=NS))
        c["attributes_with_comment"] = len(self.root.findall(".//o:EntityAttribute[a:Comment]", namespaces=NS)) +                                        len(self.root.findall(".//o:Column[a:Comment]", namespaces=NS))
        ...
        c["identifiers_primary"] = len(self.root.findall(".//o:Identifier[a:PrimaryIdentifier='1']", namespaces=NS)) +                                    len(self.root.findall(".//o:Identifier[a:PrimaryIdentifier='true']", namespaces=NS)) +                                    len(self.root.findall(".//o:Key[a:PrimaryKey='1']", namespaces=NS)) +                                    len(self.root.findall(".//o:Key[a:PrimaryKey='true']", namespaces=NS))
        return c

    def _build_extended_attribute_profile(self, extended_texts):
        ea_profile = {}
        for ext in extended_texts:
            if ext is not None and ext.text:
                parsed_attrs = self._parse_extended_attributes(ext.text)
                for prop_name, val in parsed_attrs.items():
    ...
    def _build_entities_list(self, entities):
        entities_list = []
        for e in entities:
            ...
            if ext_text is not None and ext_text.text:
                ent_dict["extended_attributes"] = self._parse_extended_attributes(ext_text.text)
                
            entities_list.append(ent_dict)
            
        return entities_list

    def _parse_extended_attributes(self, text):
        parsed = {}
        current_profile = ""
    ...
    def _write_output_files(self, outdir, counts, ea_profile, entities_list):
        if outdir is None:
