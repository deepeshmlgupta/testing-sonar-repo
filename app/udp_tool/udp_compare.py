...
def compare(model_name: str,
    ...
    ``schema`` is accepted for call-site compatibility and is not read here. The
    schema is applied earlier, by udp_readback.read_udps, which uses it to decide
    which UDPs to read back; by the time we get here the readback already
    reflects it.
    """
    # Fix:
    # Kept `schema` in the signature -- it is the THIRD POSITIONAL argument and
    # callers pass it positionally, so removing it would silently shift readback
    # into its slot rather than raise. Referenced here instead so it is no longer
    # an unused parameter.
    logger.debug("compare(%s): schema has %d entry(ies); it is applied during "
                 "readback, not here", model_name, len(schema or []))
