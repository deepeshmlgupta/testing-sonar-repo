...
def main():
    ...
    # Use the same Python installation that is running this script.
    ...
    for erwin_file in erwin_files:
        ...
        # Detect whether this is an LDM, CDM, or PDM and create its report folder.
        ...
        # Prevent an old successful run from being used in the new Excel report.
        ...
        # Only pass --results when the injector really wrote one. Passing a path
        # that does not exist would make the report claim it verified nothing,
        # which reads the same as a failed migration.
        results_args = ['--results', str(results_file)] if results_file.exists() else []
        try:
            subprocess.run(  # nosec B603
                [
                    python_executable,
                    str(base_dir / 'udp_report.py'),
                    '--model_name',
                    base_name,
                    '--manifest',
                    str(manifest_file),
        ...
        # -----------------------------------------------------------------
        # Phase 4: compare the SAP UDP values against the UDP values erwin
        # actually holds, by reading the saved model back off disk.
        #
        # This is separate from Phase 3 on purpose. Phase 3 reports what the
        # injector believed happened; Phase 4 reports what is in the file.
        # When those two disagree, the file wins - so this phase runs even if
        # Phase 2 or Phase 3 failed.
        # -----------------------------------------------------------------
        ...
        try:
            subprocess.run(  # nosec B603
                [
                    python_executable,
                    str(base_dir / 'udp_compare.py'),
                    '--model_name', base_name,
                    '--manifest', str(manifest_file),
                    '--schema', str(schema_file),
                    '--erwin', str(erwin_to_read),
                    '--sap_model', str(sap_file),
                    '--model_type', model_type,
                    '--method', 'auto',
                    '--outdir', str(model_reports_dir),
                ],
                check=True,
            )
        except subprocess.CalledProcessError:
