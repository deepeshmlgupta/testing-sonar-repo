...
def extract_pd_udps(path, unpopulated: Iterable[str] = DEFAULT_UNPOPULATED
    ...
    for ext in root.iter("{attribute}ExtendedAttributesText"):
        ...
        if owner is None or owner.get("Ref") is not None or not (ext.text or "").strip():
            continue
        tag = _local(owner.tag)
        if tag in _ENTITY_TAGS:
            obj_type, obj_name, obj_code, own_name, own_code = (
                "ENTITY", text(owner, "Name"), text(owner, "Code"), "", "")
        elif tag in _ATTRIBUTE_TAGS:
            holder = enclosing(owner, _ENTITY_TAGS)
            if holder is None:
                stats["unscoped"] += len(parse_extended_attributes_text(ext.text))
                continue
            obj_type, obj_name, obj_code = "ATTRIBUTE", text(owner, "Name"), text(owner, "Code")
            own_name, own_code = text(holder, "Name"), text(holder, "Code")
        elif owner is top_level_model or tag in _MODEL_TAGS:
            obj_type, obj_name, obj_code, own_name, own_code = (
                "MODEL", text(owner, "Name") or model.header.get("Name", ""),
                text(owner, "Code"), "", "")
        else:
            stats["unscoped"] += len(parse_extended_attributes_text(ext.text))
            continue

        for source_path, prop, value in parse_extended_attributes_text(ext.text):
...
def extract_erwin_udps(path) -> Tuple[List[UdpValue], Dict[str, int], List[str]]:
    ...
    # UDP definitions: id / Udp_Id / "Owner.Name" → display name
    definitions: Dict[str, str] = {}
    for elem in root.iter():
        if _local(elem.tag) != "Udp":
            continue
        props = next((c for c in elem if _local(c.tag).endswith("Props")), None)
        name = elem.get("name", "")
    ...
    def udp_children(props) -> List[Tuple[str, str]]:
        found = []
        for child in props:
    ...
    for elem in root.iter():
        if _local(elem.tag) == "Model":
            props = props_of(elem)
            if props is not None:
                model_name = prop_text(props, "Name") or elem.get("name", "")
                for name, value in udp_children(props):
    ...
    for entity in root.iter():
        if _local(entity.tag) not in _ERWIN_ENTITY_TAGS or entity.get("id") is None:
            continue
        e_props = props_of(entity)
        if e_props is None:
            continue
        e_name = prop_text(e_props, "Name") or entity.get("name", "")
        e_phys = prop_text(e_props, "Physical_Name")
        for name, value in udp_children(e_props):
        ...
        for group in entity:
            ...
            for attr in group:
                if _local(attr.tag) not in _ERWIN_ATTRIBUTE_TAGS:
                    continue
                a_props = props_of(attr)
                if a_props is None:
                    continue
                a_name = prop_text(a_props, "Name") or attr.get("name", "")
                a_phys = prop_text(a_props, "Physical_Name")
                for name, value in udp_children(a_props):
...
def compare_udps(pd_values: List[UdpValue], erwin_values: List[UdpValue],
    ...
    def erwin_keys(v: UdpValue):
        owner_keys = {_norm(v.owner_name), _norm(v.owner_code)} - {""} if v.object_type == "ATTRIBUTE" else {""}
    ...
    for pd in pd_values:
        ...
        elif _norm_value(erwin_hit[1], ignore_case) == _norm_value(pd.value, ignore_case):
            status, erwin_value, note = ST_MATCHED, erwin_hit[1], ""
            result.udp_matched += 1
        else:
            status, erwin_value = ST_MISMATCH, erwin_hit[1]
            note = "erwin holds a different value than SAP PD."
...
SUMMARY_HEADERS = [
    ...
    "UDP Fidelity %", "Structural Fidelity %",
]
...
def build_sheets(wb, results, config=None, tier_label: str = "") -> None:
    ...
    header(ws, ["Model", "Model Type", "UDP values (SAP PD, populated)", "Matched", "Missing",
                "Mismatch", "UDP Fidelity %", "Structural Fidelity %", "Weight",
                "Overall Fidelity %", "Unpopulated (skipped)", "erwin UDP definitions",
    ...
    for result in results:
        ...
        for col, value in enumerate(values, start=1):
            cell = ws.cell(row, col, value)
            cell.border = border
            cell.alignment = wrap if col == 14 else center
    ...
    row += 1
    header(ws, ["Model", "UDP Name", "Source Path (SAP PD)", "Values", "Matched", "Missing",
                "Mismatch", "UDP Fidelity %"], row)
    row += 1
    for result in results:
        udp = getattr(result, "udp_fidelity", None)
        if not udp:
