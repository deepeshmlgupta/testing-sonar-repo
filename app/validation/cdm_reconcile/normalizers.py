...
def describe_type(dtype: str, length: str = "", precision: str = "") -> str:
    """
    Human-readable normalised type for the report, e.g. "VARCHAR(20) / TEXT".
    Length and precision passed in separately (as PowerDesigner stores them)
    take precedence over anything glued into the type string.
    """
    if not dtype:
        return "(none)"
    ...
    if length and precision:
        ...
        dims = f"({length})"
    return f"{canonical}{dims} / {type_family(dtype)}"
