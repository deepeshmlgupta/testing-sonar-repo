...
def _build_phase_b_gate(ctx, add):
    ...
    # Fix:
    # Unpacked the nested "both / xml only / neither" ternary into a plain
    # if-elif-else. Same three outcomes in the same order, just readable one
    # branch at a time instead of a conditional inside a % argument.
    if both:
        outcome = "Both files present, gate passed."
    elif xml_ok:
        outcome = ("XML present, gate passed; the .erwin binary is absent so only "
                   "the promotion copy is affected.")
    else:
        outcome = "XML missing, gate failed - nothing downstream can run."
    add("Phase B - import gate", "B", AUTOMATED, DONE if xml_ok else FAILED,
        ...
        "continue past Phase B. It imports nothing. %s" % outcome)
    return xml_ok
...
def _non_pdm_promotion_gate(ctx, add, v):
    ...
    # Fix:
    # Lifted the nested status/detail ternaries out of the add() call. The
    # three cases -- already promoted, eligible but not copied, not eligible --
    # now sit in one if-elif-else, with status and text chosen together so they
    # cannot drift apart.
    if promoted_path:
        status = DONE
        detail = "Copied into erwinmodels/3_final - this is the blessed output."
    elif passed:
        status = PENDING
        detail = "Eligible but not copied."
    else:
        status = NA
        detail = ("Not eligible. main.py copies a model into 3_final only when status is "
                  "PASS with zero criticals; anything else stays put.")
    add("Promote to 3_final", "Gate", AUTOMATED, status, detail,
...
def _report_steps(ctx, add):
    ...
    for label, path, prov in (
        ...
        # Fix:
        # Split the nested "generated / pipeline-missing / script-missing"
        # ternary into an if-elif-else so each message stands on its own line.
        if ok:
            message = detail
        elif prov == PIPELINE:
            message = "Not generated yet — run `py -m app.main`."
        else:
            message = "Not generated yet — run the script by name."
        add(label, "Reporting", prov, DONE if ok else PENDING,
            message, path, when, size)
...
def _pdm_remediation(ctx, add, xml_ok, v, remediated):
    if remediated:
        ...
        # Fix:
        # Unpacked the nested status/detail ternaries. Note that at_target and
        # the no-XML case both report NA -- that was true before as well, it was
        # just hidden inside "NA if at_target else (PENDING if xml_ok else NA)".
        if at_target:
            status = NA
            detail = "Not needed: the model met the fidelity target on import."
        elif xml_ok:
            status = PENDING
            detail = ("Not run. py -m app.main remediates a PDM that falls short of "
                      "the target (see PDM_FIDELITY_TARGET in app/config/settings.py).")
        else:
            status = NA
            detail = "Cannot run: Phase B gate not satisfied."
        add("Remediation (preprocessing)", "C", PIPELINE, status, detail,
            expected)
