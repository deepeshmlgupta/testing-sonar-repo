...
def scan_erwin(raw: bytes):
    ...
    def start(name, attrs):
        ...
        if n in PROPS_TAGS:
            o = ErwinObject()
            o.kind = "Entity" if n == "EntityProps" else "Attribute"
...
def match_objects(pd: PDModel, objects):
    """
    Tiered match, strongest first.  Returns (matches, unmatched).
      tier 1  Name + Code      == Name + Physical_Name
      tier 2  Code             == Physical_Name        (name drifted)
      tier 3  Name             == Name, case-folded    (code drifted)
    """
    matches = {}    # id(obj) -> (comment, tier)
    unmatched = []
    ...
    for o in objects:
        ...
        if o.kind == "Entity":
            ...
            if k in pd.entities:
                hit, tier = pd.entities[k], 1
            else:
                cand = ent_by_code.get(o.physical_name, [])
                if len(cand) == 1:
                    hit, tier = pd.entities[cand[0]], 2
                else:
                    cand = ent_by_name.get((o.name or "").strip().lower(), [])
                    if len(cand) == 1:
                        hit, tier = pd.entities[cand[0]], 3
        else:
            k = (o.entity, o.name, o.physical_name)
            if k in pd.attributes:
                hit, tier = pd.attributes[k], 1
            else:
                cand = att_by_code.get((o.entity, o.physical_name), [])
                if len(cand) == 1:
                    hit, tier = pd.attributes[cand[0]], 2
                else:
                    cand = att_by_name.get(
                        (o.entity, (o.name or "").strip().lower()), [])
                    if len(cand) == 1:
                        hit, tier = pd.attributes[cand[0]], 3
...
def cmd_inspect(args):
    ...
    objs = scan_erwin(raw)
    ents = [o for o in objs if o.kind == "Entity"]
    atts = [o for o in objs if o.kind == "Attribute"]
    print()
    print("=" * 74)
    ...
    print("  with %-18s: %d / %d"
          % (ANCHOR_TAG, sum(1 for o in objs if o.anchor_end is not None), len(objs)))
    print("  already carrying %s : %d"
          % (NOTE_ARRAY_TAG, sum(1 for o in objs if o.note_array_span)))
    ...
    if not found:
        print("  no populated <Note_List> in this file "
              "(pass --reference <file with verified notes>)")
    else:
        ...
        print("  separator  : %r  (literal 6 chars; real 0x1F bytes in file: %d)"
              % (SEP, refraw.count(b"\x1f")))
        ...
        if m:
            ...
            print("  placement  : after </%s>, before <%s>"
                  % (prev[-1] if prev else "?", nxt[0] if nxt else "?"))
    ...
    tiers = Counter(t for _, t in matches.values())
    print("  matched          : %d / %d" % (len(matches), len(objs)))
    for t in sorted(tiers):
        print("     tier %d        : %d" % (t, tiers[t]))
    print("  unmatched        : %d" % len(unmatched))
    print("  matched & has PD Comment : %d"
          % sum(1 for c, _ in matches.values() if c.strip()))
    for o in unmatched[:10]:
        print("     UNMATCHED     : %s (physical=%r)" % (o.label, o.physical_name))
    return 0
...
def cmd_migrate(args):
    ...
    only = set(args.only.split(",")) if args.only else {"Entity", "Attribute"}

    edits = []      # (start, end, replacement_bytes)
    rows = []
    stats = Counter()

    for o in objects:
        row = {"kind": o.kind, "entity": o.entity or "", "name": o.name or "",
               "physical_name": o.physical_name or "", "status": "",
               "match_tier": "", "reason": "", "comment_chars": 0}

        if id(o) not in matches:
            row["status"] = "UNMATCHED"
            row["reason"] = "no PD object with this Name/Code"
            stats["unmatched"] += 1
            rows.append(row)
            continue
        ...
        if o.kind not in only:
            row["status"] = "SKIPPED"
            row["reason"] = "object kind excluded by --only"
            stats["skipped"] += 1
            rows.append(row)
            continue
        ...
        if o.existing_notes and args.on_existing == "skip":
            row["status"] = "SKIPPED"
            row["reason"] = "erwin Note already present (--on-existing skip)"
            stats["skipped"] += 1
            rows.append(row)
            continue
        ...
        if o.existing_notes and args.on_existing == "append":
            ...
            new = build_note_record(comment, args.author, args.newline_mode,
                                    number=len(recs) + 1)
            payload = build_note_array(
                [_unescape(r) for r in recs] + [new]).encode("utf-8")
            edits.append((start, end, payload))
            row["reason"] = "appended as note #%d" % (len(recs) + 1)

        elif o.note_array_span:
            # replace an existing (possibly empty) Note_List_Array
            start, end = o.note_array_span
            rec = build_note_record(comment, args.author, args.newline_mode)
            edits.append((start, end, build_note_array([rec]).encode("utf-8")))
            row["reason"] = ("replaced existing Note_List_Array"
                             if o.existing_notes else "filled empty Note_List_Array")

        else:
            pos = o.anchor_end if o.anchor_end is not None else o.props_close
            if pos is None:
                row["status"] = "SKIPPED"
                row["reason"] = "no insertion anchor found"
                stats["skipped"] += 1
                rows.append(row)
                continue
            rec = build_note_record(comment, args.author, args.newline_mode)
            edits.append((pos, pos, build_note_array([rec]).encode("utf-8")))
            row["reason"] = "inserted after </%s>" % ANCHOR_TAG
    ...
    out = bytearray(raw)
    for start, end, payload in sorted(edits, key=lambda e: e[0], reverse=True):
    ...
    if args.report:
        with open(args.report, "w", newline="", encoding="utf-8") as fh:
            w = csv.DictWriter(fh, fieldnames=list(rows[0].keys()) if rows else
                               ["kind", "entity", "name", "physical_name",
                                "status", "match_tier", "reason", "comment_chars"])
            w.writeheader()
            w.writerows(rows)
    ...
    print("=" * 74)
    print("  LDM                 : %s" % args.ldm)
    print("  erwin in            : %s  (%d bytes)" % (args.erwin, len(raw)))
    print("  erwin out           : %s  (%d bytes)" % (args.out, len(out)))
    print("  newline mode        : %s" % args.newline_mode)
    print("  note author         : %s" % args.author)
    print()
    print("  erwin objects       : %d  (Entity %d / Attribute %d)"
          % (len(objects),
             sum(1 for o in objects if o.kind == "Entity"),
             sum(1 for o in objects if o.kind == "Attribute")))
    print("  matched             : %d" % stats["matched"])
    print("  updated (Note set)  : %d" % stats["updated"])
    print("  skipped             : %d" % stats["skipped"])
    print("  unmatched           : %d" % stats["unmatched"])
    reasons = Counter(r["reason"] for r in rows if r["status"] == "SKIPPED")
    for reason, n in reasons.most_common():
        print("       skipped: %-45s %d" % (reason, n))
    if unmatched:
        print("  unmatched objects (first 10):")
        for o in unmatched[:10]:
            print("       %s  physical=%r" % (o.label, o.physical_name))
    if args.report:
        print()
        print("  per-object report   : %s" % args.report)
    print()
    print("  Next: run `verify` to prove only Notes changed.")
    return 0
...
def cmd_verify(args):
    ...
    # ---- Check 1: byte-level. Strip all Note_List_Array regions from both. ----
    a_stripped = NOTE_ARRAY_RE.sub(b"", a)
    b_stripped = NOTE_ARRAY_RE.sub(b"", b)
    same = a_stripped == b_stripped
    ok &= same
    print("  [%s] byte-level: everything outside Note_List_Array is identical"
    ...
    # ---- Check 2: structural, property by property ----
    try:
    ...
    def collect(path):
        ...
        for el in tree.getroot().iter():
            if localname(el.tag) not in ("Entity", "Attribute"):
                continue
            props = None
            for ch in el:
            ...
            d = {}
            for pr in props:
                d.setdefault(localname(pr.tag), []).append(
                    etree.tostring(pr, encoding="unicode"))
            out[(localname(el.tag), el.get("id"))] = d
        return out

    A, B = collect(args.erwin), collect(args.out)

    same_keys = set(A) == set(B)
    ok &= same_keys
    print("  [%s] same object set: %d objects in / %d out"
          % ("PASS" if same_keys else "FAIL", len(A), len(B)))
    ...
    print("  [%s] properties added   : %s"
          % ("PASS" if not illegal_add else "FAIL",
             dict(added) if added else "{}"))
    print("  [%s] properties removed : %s"
          % ("PASS" if not removed else "FAIL", dict(removed) if removed else "{}"))
    print("  [%s] properties changed : %s"
          % ("PASS" if not illegal_chg else "FAIL",
             dict(changed) if changed else "{}"))
    ok &= not illegal_add and not removed and not illegal_chg

    # ---- Check 3: Definition and other protected fields, explicitly ----
    ...
    for k in set(A) & set(B):
        for prop in protected:
            if A[k].get(prop) != B[k].get(prop):
                bad.append((k, prop))
    ok &= not bad
    print("  [%s] protected fields unchanged (%s)"
          % ("PASS" if not bad else "FAIL", ", ".join(protected[:5]) + ", ..."))
    for k, prop in bad[:10]:
        print("        CHANGED %s %s -> %s" % (k, prop, prop))

    # ---- Check 4: note payload sanity ----
    notes = 0
    badrec = 0
    tree = etree.parse(args.out, p)
    for el in tree.getroot().iter():
        ...
        if d["field_count"] != NOTE_FIELD_COUNT:
            badrec += 1
    print("  [%s] note records well-formed: %d notes, %d malformed"
          % ("PASS" if badrec == 0 else "FAIL", notes, badrec))
    ok &= badrec == 0

    print()
    print("  RESULT: %s" % ("PASS - only Notes were changed" if ok else "FAIL"))
    return 0 if ok else 1
